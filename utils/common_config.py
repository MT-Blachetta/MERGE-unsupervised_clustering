...
def get_model_lightly(p,pretrain_path):
    ...
    elif p['backbone'] == 'clPcl':
        from model.resnet_stl import resnet18
        model = resnet18()
    ...
    backbone_model = {'backbone': backbone, 'dim': p['out_dim'] }
    from models.models import ClusteringModel_lightly
    model = ClusteringModel_lightly(backbone_model, p['num_classes'], p['num_heads'])
    ...
    if pretrain_path is not None and os.path.exists(pretrain_path):
        ...
        if p['setup'] == 'scan': # Weights are supposed to be transfered from contrastive training
            ...
            assert(set(missing[1]) == {
                ...
                or set(missing[1]) == {'contrastive_head.bias', 'contrastive_head.weight', 'fc.bias', 'fc.weight'})
