...
@torch.no_grad()
def get_predictions(device_id, p, dataloader, model, return_features=False):
    ...
    if isinstance(dataloader.dataset, NeighborsDataset): # Also return the neighbors
        ...
        #print('TYP: ',type(dataloader.dataset))
        key_ = 'image'
...
@torch.no_grad()
def hungarian_evaluate(device_id,subhead_index, all_predictions, class_names=None, 
    ...
    predictions = predictions.to(device_id)
    probs = head['probabilities']
    probs = probs.to(device_id)
    ...
    reordered_preds = reordered_preds.to(device_id)
